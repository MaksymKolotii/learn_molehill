package {
	...
	public class Stage3dParticle extends Stage3dEntity {
		...
		private var ageScale:Vector.<Number> = new <Number>[1, 0, 1, 1];
		private var rgbaScale:Vector.<Number> = new <Number>[1, 1, 1, 1];
		...
		public function Stage3dParticle(mydata:Class = null, mycontext:Context3D = null, mytexture:Texture = null, mydata2:Class = null) {
			...
			} else if (context) { // only one mesh defined: use the simpler shader
			...
			if (mydata2 && context) {
				mesh2 = new Stage3dObjParser(mydata2, context, 1, true, true);
			}
		...
		private function wobble(ms:Number = 0, amp:Number = 1, spd:Number = 1):Number {
			return amp * Math.sin((ms / 1000) * spd * twoPi);
		}
		...
		private function wobble010(ms:Number):Number {
			return wobble(ms - 250, 0.5, 1.0) + 0.5;
		}
		...
		private function initParticleShader(twomodels:Boolean = false):void {
			...
			if (twomodels) {
				if (particleshader2mesh) { // already compiled previously?
				...
				if (particleshader1mesh) { // already compiled previously?
